...
type ParticipationView = {
  ...
  joinedSnapshot: { version: number; dataType: string; gatewayUrl: string; publicData: { monitoredReduction?: { value: string }; projectProgress?: string | null } };
  latestSnapshot: { version: number; verificationStage: string; publicData: { monitoredReduction?: { value: string }; projectProgress?: string | null } } | null;
};
...
export function MyPage() {
  ...
  async function deleteAccount() {
    const confirmed = window.confirm("Delete your EGOG and Privy account connection? Your public on-chain badge and wallet address cannot be deleted, and you may lose access to this embedded wallet.");
    if (!confirmed) return;
    setDeleting(true);
    try {
      const token = await getAccessToken();
      if (!token) throw new Error("Missing access token");
      const response = await fetch("/api/me", { method: "DELETE", headers: { authorization: `Bearer ${token}` } });
  ...
  if (!ready || loading) return <div className="account-empty"><span className="spinner-large" /><h1>Loading My Participation…</h1></div>;
  ...
  return (
    <div className="account-layout">
      ...
      {items.length === 0 ? <section className="account-empty compact"><ShieldCheck size={32} /><h2>No participation badge yet</h2><p>Explore an active project and sign your support to create your first record.</p><Link className="primary-cta" href="/">Explore projects</Link></section> : <div className="participation-list">{items.map((item) => <article className="participation-record" key={item.participation.transactionHash}><div className="participation-art"><Image alt="EGOG participation badge" height={220} src="/images/participation-badge.svg" width={220} /></div><div className="participation-copy"><div><span className="demo-pill">{item.joinedSnapshot.dataType} data at join</span><h2>{item.project.name}</h2><p>Early Participant #{item.participation.memberNumber} · Confirmed</p><small>Joined {new Date(item.participation.joinedAt).toLocaleString("en-GB")}</small></div><dl><div><dt>Joined Snapshot</dt><dd>v{item.joinedSnapshot.version} · {item.joinedSnapshot.publicData.monitoredReduction?.value ?? "—"} tCO₂e</dd></div><div><dt>Latest Snapshot</dt><dd>v{item.latestSnapshot?.version ?? "—"} · {item.latestSnapshot?.publicData.monitoredReduction?.value ?? "—"} tCO₂e</dd></div><div><dt>Current stage</dt><dd>{item.latestSnapshot?.verificationStage ?? "Not available"}</dd></div><div><dt>Current progress</dt><dd>{item.latestSnapshot?.publicData.projectProgress ? `${item.latestSnapshot.publicData.projectProgress}%` : "Not available"}</dd></div><div><dt>NFT Token ID</dt><dd>{item.participation.tokenId}</dd></div></dl><div className="record-actions"><a href={item.participation.tokenUri.replace("ipfs://", `${clientEnvironment.NEXT_PUBLIC_PINATA_GATEWAY_URL}/`)} target="_blank" rel="noreferrer">View badge <ExternalLink size={13} /></a><Link href={`/projects/${item.project.slug}`}>View project</Link><a href={`${clientEnvironment.NEXT_PUBLIC_GIWA_EXPLORER_URL}/tx/${item.participation.transactionHash}`} target="_blank" rel="noreferrer">Participation record <ExternalLink size={13} /></a><a href={item.joinedSnapshot.gatewayUrl} target="_blank" rel="noreferrer">Snapshot JSON <ExternalLink size={13} /></a></div></div></article>)}</div>}
      ...
      <section className="account-danger"><div><h2>Delete account</h2><p>Off-chain personal data and your Privy connection will be removed. Public blockchain records and badges remain permanently visible.</p></div><button disabled={deleting} onClick={deleteAccount} type="button"><Trash2 size={16} /> {deleting ? "Deleting…" : "Delete account"}</button></section>
    </div>
  );
}
